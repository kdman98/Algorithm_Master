...
void checkPuzzle(int puzzle[][size]) 
{
	...
	for(int i=0;i<size;i++){
		for(int j=0;j<size;j++){
			if(puzzle[i][j] != (size*size-1-(i*size)-j)){
				return;
			}
		}
	}
...
int move_to(int puzzle[][size],int loc,int command)
{
	...
	if(command==0){
		if(loc < (size-1)*(size) ){
			puzzle[loc/size][loc%size]=puzzle[(loc/size)+1][loc%size];
		...
		if(loc%size<size-1){
			puzzle[loc/size][loc%size]=puzzle[loc/size][loc%size+1];
    ...
    {  
		...
		*/
		...
		/*
		   ...
		   key에 대한 동작을 수행한다.
		*/
		if(move_to(puzzle,find_0_loc(puzzle),command)>0) life--;
